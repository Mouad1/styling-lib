...
@Component({
  selector: 'app-table-domain-header',
  templateUrl: './table-domain-header.component.html',
  styleUrls: ['./table-domain-header.component.scss'],
})
export class TableDomainHeaderComponent {
  msgs: Message[] = [];
  ...
  selectedBrowsers: Browser[] | undefined;

  displayDialog: boolean | undefined;

  stacked: boolean | undefined;

  newBrowser: boolean | undefined;

  totalRecords: number = 100;

  engines: SelectItem[] | undefined;

  grades: SelectItem[] | undefined;

  expandedRows: any[] | undefined;

  cols: any[] | undefined;

  columnOptions: SelectItem[] | undefined;
  ...
  ngOnInit() {
    this.browserService
      .getBrowsers()
      .subscribe((browsers: any) => (this.browsers = browsers));
    this.browserService
      .getBrowsers()
      .subscribe(
        (browsers: any) => (this.basicBrowsers = browsers.slice(0, 10))
      );
    this.cols = [
  ...
  onRowClick(event: any) {
    ...
    this.msgs.push({
      severity: 'info',
      summary: 'Browser clicked',
      detail: event.data.engine + ' - ' + event.data.browser,
    });
  }
  ...
  onRowDblClick(event: any) {
    ...
    this.msgs.push({
      severity: 'info',
      summary: 'Browser double clicked',
      detail: event.data.engine + ' - ' + event.data.browser,
    });
  }
  ...
  onHeaderCheckboxToggle(event: any) {
    ...
    this.msgs.push({
      severity: 'info',
      summary: 'Header checkbox toggled:',
  ...
  onColResize(event: any) {
    ...
    this.msgs.push({
      severity: 'info',
      summary: 'Resized column header' + event.element,
  ...
  onColReorder(event: any) {
    ...
    this.msgs.push({
      severity: 'info',
      summary: 'Index of dragged column',
      detail: event.dragIndex,
    });
    this.msgs.push({
      severity: 'info',
      summary: 'Index of dropped column',
      detail: event.dropIndex,
    });
    this.msgs.push({
      severity: 'info',
      summary: 'Columns array after reorder',
      detail: event.columns,
    });
  }
  ...
  onSort(event: any) {
    ...
    if (event.multisortmeta) {
      this.msgs.push({
        severity: 'info',
        summary: 'Multisort Meta data:',
        detail: event.multisortmeta,
      });
    }
  }
  ...
  onFilter(event: any) {
    ...
    this.msgs.push({
      severity: 'info',
      summary: 'Filter object(field,value and matchmode):',
  ...
  selectBrowser(browser: Browser) {
    ...
    this.msgs.push({
      severity: 'info',
      summary: 'Browser selected',
      detail: 'Browser: ' + browser.browser,
    });
  }
